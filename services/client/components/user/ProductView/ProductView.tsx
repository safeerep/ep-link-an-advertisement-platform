...
import { authRequired, getSpecificProduct } from '@/store/actions/userActions/userActions'
        ...
        <>
            <div className="p-4 pt-6 bg-slate-50">
                <div className="w-full flex justify-center">
                    <img className="sm:w-full md:w-1/2 lg:w-3/4 h-96 p-4 border border-black object-fill" src={product?.images ? `${PRODUCT_IMAGES_URL}/${product?.images[0]}` : ''} alt="" />
                    <div className='bg-slate-100 sm:w-full md:w-1/2 lg:w-1/4'>
                        {product && Object?.keys(product?.inputFields).map(key => (
                            <div key={key}>
                                <h3>{key}</h3>
                                <p>{product?.inputFields[key]}</p>
                            </div>
                        ))}
                    ...
                    <div className=" bg-white p-3 shadow ">
                        ...
                        <>
                            ...
                            <div className='w-full flex justify-center items-center gap-2'>
                                {sellerStatus === "following" && (
                                    <button className='bg-slate-950 px-4 rounded-md'>
                                        <span className='px-4 text-teal-50' >UNFOLLOW</span>
                                    </button>
                                )}
                                {sellerStatus === "not-following" && (
                                    <button className='bg-slate-950 px-4 rounded-md'>
                                        <span className='px-4 text-teal-50' >FOLLOW</span>
                                    </button>
                                )}
                                <button className='bg-slate-700 p-1 text-white'>
                                    <BsChatDots />
                                </button>
                            </div>
                        </>
